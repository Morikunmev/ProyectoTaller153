...
export const useAdminUpdateModal = ({
  isOpen,
  onClose,
  admin,
  onAdminUpdated,
  ...
  useEffect(() => {
    if (admin && isOpen) {
      setFormData({
        ...
        TelefonoUsuario: admin.telefono || "",
        EdadUsuario: admin.edad || "",
        password: "",
        FotoUsuario: null,
      });
      setPreviewUrl(admin.foto || "");
    ...
    async (e) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/administrador/${admin.id}/actualizar/`,
          {
            method: "POST",
            headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(data.message || "Error al actualizar el administrador");
        }

        handleClose();
        if (onAdminUpdated) {
          await onAdminUpdated(data.administrador);
        }
      } catch (error) {
        console.error("Error al actualizar el administrador:", error);
        setErrors((prev) => ({
          ...prev,
          general: error.message || "Error al actualizar el administrador",
        }));
      } finally {
        setIsSubmitting(false);
      }
    },
    [formData, admin, handleClose, onAdminUpdated, removedFiles]
  );
...
export default useAdminUpdateModal;