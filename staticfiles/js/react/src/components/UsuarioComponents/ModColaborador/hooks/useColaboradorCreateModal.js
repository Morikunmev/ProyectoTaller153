...
export const useColaboradorCreateModal = ({ isOpen, onClose, onSubmit }) => {
 // Form Data State - Solo inicializamos los campos según el modelo de Usuario/Colaborador 
 ...
 const handleSubmit = async (e) => {
   ...
   try {
     ...
     const response = await fetch("/api/colaborador/crear/", {
       method: "POST",
       body: formDataToSend,
       headers: {
     ...
     if (!response.ok) {
       ...
       throw new Error(data.message || "Error al crear el colaborador");
     }

     if (data.success) {
       onSubmit(data.colaborador);
       handleClose();
     }
   } catch (error) {
     ...
     setErrors((prev) => ({
       ...prev,
       general: error.message || "Error al crear el colaborador",
     }));
   } finally {
