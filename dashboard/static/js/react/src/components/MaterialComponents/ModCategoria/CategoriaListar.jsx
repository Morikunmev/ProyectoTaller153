...
const CategoriaListar = ({ isModalOpen, setIsModalOpen }) => {
  const {
    searchTerm,
    loading,
    error,
    ...
    <table className="w-full">
      <thead>
        <tr className="text-left text-gray-500 text-sm border-b bg-gray-50">
          ...
          <th className="p-2 font-medium">PRODUCTOS PERDIDOS</th>
          <th className="p-2 font-medium">PRODUCTOS VENDIDOS</th>
          <th className="p-2 font-medium">TOTAL VENTAS</th>
          <th className="p-2 font-medium">TOTAL PERDIDAS</th>
      ...
      <tbody>
        {currentCategorias.map((categoria) => (
          <tr key={categoria.id} className="border-b hover:bg-gray-50">
            <td className="p-2 font-medium text-gray-900">#{categoria.id}</td>
            <td className="p-2">
              <div className="w-8 h-8 rounded-full overflow-hidden bg-gray-100 flex items-center justify-center">
                {categoria.FotoCategoria ? (
                  <img
                    src={categoria.FotoCategoria}
                    alt={`Categoria: ${categoria.NombreCategoria}`}
            ...
            <td className="p-2 font-medium">{categoria.NombreCategoria}</td>
            <td
              className="p-2 max-w-xs truncate"
              title={categoria.DescripcionCategoria}
            >
              {categoria.DescripcionCategoria || "Sin descripción"}
            </td>
            <td className="p-2">{categoria.StockCategoria}</td>
            <td className="p-2">{categoria.CantidadCategoriaPerdida}</td>
            <td className="p-2">{categoria.CantidadCategoriaVenta}</td>
            <td className="p-2">
            ...
            <td className="p-2">
              <div className="flex justify-center gap-1">
                <button
                  type="button"
                  onClick={() => handleUpdateModalOpen(categoria)}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-4">
      {currentCategorias.map((categoria) => (
        <CategoriaGrid
          key={categoria.id}
          categoria={categoria}
          onEdit={() => handleUpdateModalOpen(categoria)}
  ...
  return (
    <div>
            ...
            <div className="flex items-center space-x-3 max-[790px]:w-full">
              ...
              >
                <span className="flex items-center max-[790px]:justify-center">
                  ...
                  <span className="ml-1">Crear Categoría</span>
                </span>
              </button>
            </div>
          </div>
      ...
      <CategoriaCreateModal
        ...
        onSubmit={handleCategoriaCreated}
      />
      <CategoriaDeleteModal
        ...
        categoriaId={categoriaToDelete?.id}
        isDeleting={isDeleting}
      />

      <CategoriaUpdateModal
